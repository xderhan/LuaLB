#ifdef HAVE_CONFIG_H
#include "config.h"
#endif /* HAVE_CONFIG_H */

#include <string.h>
#include <math.h>
#include <hdf5.h>

#include "lb-d2q9.h"
#include "lb-d2q9-utils.h"
#include "lb-memory.h"
#include "lb-messages.h"

#ifdef LB_ENABLE_MPI
#include <mpi.h>
#include "lb-mpi-utils.h"
#endif /* LB_ENABLE_MPI */

#include "lb-d2q9-BGK.h"

struct _LBD2Q9BGK {
	LBD2Q9          d2q9;
	LBGKParameters parameters;
	double*	pdfs[9];
	double*	rho;
	double*	u;
	double	ux_b1;
	double	ux_b3;
#ifdef LB_ENABLE_MPI
	MPI_Datatype	col_type;
	MPI_Datatype	row_type;
#endif /* LB_ENABLE_MPI */
};

/*
 *  handy macros
 */

#define NX (self->d2q9.partition_info.size[0])
#define NY (self->d2q9.partition_info.size[1])
#define RANK (self->d2q9.particle_rank)
#define OFFSET(x,y) RANK*( (x)*(NY + 2) + (y) )

/*
 *  private methods
 */

static void
lb_d2q9_BGK_average_pdfs_1(LBD2Q9BGK* self, int o)
{
	double t1 = self->pdfs[0][o] + self->pdfs[7][o];
	double t2 = self->pdfs[1][o] + self->pdfs[5][o];
	double t3 = self->pdfs[4][o] - self->pdfs[6][o];
	
	double rho = t1 + t2 + self->pdfs[2][o]
					 + self->pdfs[3][o] + self->pdfs[4][o]
					 + self->pdfs[6][o] + self->pdfs[8][o];
	
	self->rho[o] = rho;

	self->u[2*o + 0] = (t1 + t3 - self->pdfs[2][o] - self->pdfs[5][o])/rho;
	self->u[2*o + 1] = (t2 + t3 - self->pdfs[3][o] - self->pdfs[7][o])/rho;
}

static void
lb_d2q9_BGK_average_pdfs(LBD2Q9BGK* self)
{
	int x, y, c;

	lb_assert(self != NULL);

	for (x = 1; x <= NX; ++x)
		for (y = 1; y <= NY; ++y)
			for (c = 0; c < RANK; ++c)
				lb_d2q9_BGK_average_pdfs_1(self, OFFSET(x, y) + c);
}

#ifdef LB_ENABLE_MPI
static void
lb_d2q9_BGK_exchange_rho(LBD2Q9BGK* self)
{
	const int todo[4][4] = {
		{NX, 1, NX + 1, 1},
		{1, NY, 1, NY + 1},
		{1, 1, 0, 1},
		{1, 1, 1, 0}
	};

	int n, nr;

	for (n = 0, nr = 0; n < 4; ++n) {
		int rank = lb_d2q9_need_exchange(&self->d2q9, n);

		if (rank != MPI_PROC_NULL) {
			double* send = self->rho
				     + OFFSET(todo[n][0], todo[n][1]);

			double* recv = self->rho
				     + OFFSET(todo[n][2], todo[n][3]);

			MPI_Datatype type =
				(n % 2 ? self->row_type : self->col_type);

			LB_CALL_MPI(MPI_Isend(send, 1, type, rank, n,
				self->d2q9.comm, self->d2q9.requests + nr++))

			LB_CALL_MPI(MPI_Irecv(recv, 1, type, rank, (n + 2)%4,
				self->d2q9.comm, self->d2q9.requests + nr++))
		}
	}

	if (nr > 0) {
		MPI_Status* statuses = lb_newa(MPI_Status, nr);

		LB_CALL_MPI(MPI_Waitall(nr, self->d2q9.requests, statuses))
	}
}
#endif /* LB_ENABLE_MPI */

static void
lb_d2q9_BGK_wrap_rho(LBD2Q9BGK* self)
{
	lb_assert(self != NULL);

#ifdef LB_ENABLE_MPI
        if (self->d2q9.neighbors[0] ==
	    self->d2q9.partition_info.processor_rank) {
#else
        if (self->d2q9.partition_info.periods[0]) {
#endif /* LB_ENABLE_MPI */
		lb_scpy(self->rho + OFFSET(NX + 1, 1),
			  self->rho + OFFSET(1, 1),
			  1, 1, NY);

		lb_scpy(self->rho + OFFSET(0, 1),
			  self->rho + OFFSET(NX, 1),
			  1, 1, NY);
	}

#ifdef LB_ENABLE_MPI
        if (self->d2q9.neighbors[1] ==
	    self->d2q9.partition_info.processor_rank) {
#else
        if (self->d2q9.partition_info.periods[1]) {
#endif /* LB_ENABLE_MPI */
		lb_scpy(self->rho + OFFSET(1, NY + 1),
			  self->rho + OFFSET(1, 1),
			  1, (NY + 2), NX);

		lb_scpy(self->rho + OFFSET(1, 0),
			  self->rho + OFFSET(1, NY),
			  1, (NY + 2), NX);
	}
}

static void
lb_d2q9_BGK_setup_rho_guards(LBD2Q9BGK* self)
{
	lb_assert(self != NULL);

#ifdef LB_ENABLE_MPI
	lb_d2q9_BGK_exchange_rho(self);
#endif /* LB_ENABLE_MPI */
	lb_d2q9_BGK_wrap_rho(self);
}

static inline void
lb_d2q9_BGK_collide_node_pdfs(LBD2Q9BGK* self, int o,
				const double* LB_RESTRICT ctau)
{
	int n;
	double eq[9];
	const double ohm = 1./(*ctau);
	const double alp = 1. - 1./(*ctau);

	lb_d2q9_BGK_equilibrium(&self->parameters,
				  self->rho + o, self->u + 2*o, eq);

	for (n = 0; n < 9; ++n) {
		const double tmp = self->pdfs[n][o];
		self->pdfs[n][o] = alp*tmp + ohm*eq[n];
	}
}

static inline void
lb_d2q9_BGK_apply_bc(double* LB_RESTRICT rho,
						double* pdfs[9], int o, int mode)
{
	/*static const int V[4][2] = {
		{1, 1}, {-1, 1}, {-1, -1}, {1, -1}
	};*/

	//const double r2 = 0.5*rho[0];
	//const double ux = rho[1] - *U;
	//const double uy = rho[2];

	const double t1 = 0.5*(pdfs[0][o] - pdfs[2][o]);

	//const double t1 = r2*(ux*V[v1 - 4][0] + uy*V[v1 - 4][1]);
	//const double t2 = r2*(ux*V[v2 - 4][0] + uy*V[v2 - 4][1]);

	//pdfs[v1][o] -= t1;
	//pdfs[v2][o] -= t2;

	if (mode == 1)
	{
		pdfs[3][o] = pdfs[1][o];
		pdfs[6][o] = pdfs[4][o] + t1;
		pdfs[7][o] = pdfs[5][o] - t1;
		rho[0] = pdfs[8][o] + pdfs[0][o] + pdfs[2][o] + 2.*(pdfs[1][o] + pdfs[4][o] + pdfs[5][o]);
		rho[1] = 0.;
		rho[2] = 0.;
	}

	if (mode == 3)
	{
		pdfs[1][o] = pdfs[3][o];
		pdfs[4][o] = pdfs[6][o] - t1;
		pdfs[5][o] = pdfs[7][o] + t1;
		rho[0] = pdfs[8][o] + pdfs[0][o] + pdfs[2][o] + 2.*(pdfs[3][o] + pdfs[6][o] + pdfs[7][o]);
		rho[1] = 0.;
		rho[2] = 0.;
	}

	//pdfs[8][o] += t1 + t2;

	//rho[1] = *U;
	//rho[2] = 0.0;
}

static void
lb_d2q9_BGK_collide_pdfs(LBD2Q9BGK* self, double ctau)
{
	//const int y0 = (lb_d2q9_is_boundary(&self->d2q9, 3) ? 2 : 1);
	//const int y1 = (lb_d2q9_is_boundary(&self->d2q9, 1) ? NY - 1 : NY);

	const int y0 = 1;
	const int y1 = NY;

	int x, y, c;

	/*
	 *  bulk sites first
	 */

	for (x = 1; x <= NX; ++x)
		for (y = y0; y <= y1; ++y)
			for (c = 0; c < RANK; ++c) {
				const int offset = OFFSET(x, y) + c;
				lb_d2q9_BGK_collide_node_pdfs(self, offset, &ctau);
			}

	/*
	 *  boundary (if any) 
	 */

	/*if (lb_d2q9_is_boundary(&self->d2q9, 3))
		for (x = 1; x <= NX; ++x) {
			const int offset = OFFSET(x, 1);
	
			lb_d2q9_BGK_apply_bc(self->rho + 3*offset,
									self->pdfs, offset, 3);

			lb_d2q9_BGK_collide_node_pdfs(self, offset, &ctau);
		}

	if (lb_d2q9_is_boundary(&self->d2q9, 1))
		for (x = 1; x <= NX; ++x) {
			const int offset = OFFSET(x, NY);

			lb_d2q9_BGK_apply_bc(self->rho + 3*offset,
									self->pdfs, offset, 1);

			lb_d2q9_BGK_collide_node_pdfs(self, offset, &ctau);
		}*/
}

/*
 *  public interface
 */

LBD2Q9BGK*
LBD2Q9BGK_new(int c, int nx, int ny, int px, int py)
{
	int n, nnodes;
	LBD2Q9BGK* self;

	lb_assert(nx > 2 && ny > 2);

	self = lb_new(LBD2Q9BGK, 1);

#ifdef LB_ENABLE_MPI
	lb_d2q9_init(&self->d2q9, MPI_COMM_WORLD, c, nx, ny, px, py);

	LB_CALL_MPI(MPI_Type_vector(NY, c, 1,
				      MPI_DOUBLE, &self->col_type))
	LB_CALL_MPI(MPI_Type_commit(&self->col_type))

	LB_CALL_MPI(MPI_Type_vector(NX, c, (NY + 2),
				      MPI_DOUBLE, &self->row_type))
	LB_CALL_MPI(MPI_Type_commit(&self->row_type))
#else
	lb_d2q9_init(&self->d2q9, c, nx, ny, px, py);
#endif /* LB_ENABLE_MPI */

	self->parameters.T = 1.0;
	self->parameters.a = 0.0;
	self->parameters.b = 0.0;
	self->parameters.K = 0.01;
	self->parameters.tau = 0.8;

	if (NX < 3 || NY < 3)
		lb_fatal("D2Q9BGK: size of the grid too small"
#ifdef LB_ENABLE_MPI
			" (or too much MPI processes)"
#endif /* LB_ENABLE_MPI */
			);

	nnodes = RANK*(NX + 2)*(NY + 2);

	for (n = 0; n < 9; ++n)
		self->pdfs[n] = lb_new(double, nnodes);

	self->rho = lb_new(double, nnodes);
	self->u = lb_new(double, 2*nnodes);

	self->ux_b1 =
	self->ux_b3 = 0.0;

#ifdef LB_ENABLE_MPI
	LB_CALL_MPI(MPI_Barrier(self->d2q9.comm))
#endif /* LB_ENABLE_MPI */

	return self;
}

void
LBD2Q9BGK_destroy(LBD2Q9BGK* self)
{
	int n;

	lb_assert(self != NULL);

	lb_free(self->rho);
	lb_free(self->u);
	
	for (n = 0; n < 9; ++n)
		lb_free(self->pdfs[n]);

#ifdef LB_ENABLE_MPI
	LB_CALL_MPI(MPI_Type_free(&self->row_type))
	LB_CALL_MPI(MPI_Type_free(&self->col_type))
#endif /* LB_ENABLE_MPI */

	lb_d2q9_fini(&self->d2q9);

	lb_free(self);
}

void
LBD2Q9BGK_partition_info(const LBD2Q9BGK* self,
			    LBPartitionInfo* pinfo)
{
	lb_assert(self != NULL);
	lb_assert(pinfo != NULL);

	*pinfo = self->d2q9.partition_info;
}

/*
LBPartitionInfo*
LBD2Q9BGK_partition_info(const LBD2Q9BGK* self)
{
	LBPartitionInfo* pinfo;

	lb_assert(self != NULL);

	pinfo = lb_new(LBPartitionInfo, 1);

	_LBD2Q9BGK_partition_info(self, pinfo);

	return pinfo;
}
*/

void
LBD2Q9BGK_set_parameters(LBD2Q9BGK*             self,
			     const LBGKParameters* parameters)
{
	lb_assert(self != NULL);
	lb_assert(parameters != NULL);
	lb_assert(parameters->tau > 0.5);

	self->parameters = *parameters;
}

void
LBD2Q9BGK_get_parameters(const LBD2Q9BGK* self,
			     LBGKParameters* parameters)
{
	lb_assert(self != NULL);
	lb_assert(parameters != NULL);

	*parameters = self->parameters;
}

/*
LBGKParameters*
LBD2Q9BGK_get_parameters(const LBD2Q9BGK* self)
{
	LBGKParameters* params;

	lb_assert(self != NULL);

	params = lb_new(LBGKParameters, 1);

	lb_d2q9_get_parameters(self, params);

	return params;
}
*/

void
LBD2Q9BGK_set_walls_speed(LBD2Q9BGK* self, double vx_top, double vx_down)
{
	lb_assert(self != NULL);

	if (self->d2q9.partition_info.periods[1])
		lb_warning("LBD2Q9BGK: setting speed of the walls will"
			    " not have any effect because of periodic boundary"
			    " conditions\n");

	self->ux_b1 = vx_top;
	self->ux_b3 = vx_down;
}

void
LBD2Q9BGK_get_walls_speed(const LBD2Q9BGK* self,
			    double* vx_top, double* vx_down)
{
	lb_assert(self != NULL);
	lb_assert(vx_top && vx_down);

	*vx_top = self->ux_b1;
	*vx_down = self->ux_b3;
}

void 
LBD2Q9BGK_set_pdfs(LBD2Q9BGK* self, int a, int x, int y, int c, double f) 
{
	lb_assert(self != NULL);
	lb_assert(x >= 0 && x < NX);
	lb_assert(y >= 0 && y < NY);
	lb_assert(f >= 0.0);
	
	self->pdfs[a][OFFSET(x + 1, y + 1) + c] = f;
}

void 
LBD2Q9BGK_get_pdfs(const LBD2Q9BGK* self, int a, int x, int y, int c, double* f) 
{
	lb_assert(self != NULL);
	lb_assert(x >= 0 && x < NX);
	lb_assert(y >= 0 && y < NY);
	lb_assert(f);
	
	*f = self->pdfs[a][OFFSET(x + 1, y + 1) + c];
}

void
LBD2Q9BGK_set_averages(LBD2Q9BGK* self, int x, int y, int c,
			double rho, double ux, double uy)
{
	double* ptr;

	lb_assert(self != NULL);
	lb_assert(x >= 0 && x < NX);
	lb_assert(y >= 0 && y < NY);

	const int o = OFFSET(x + 1, y + 1) + c;

	ptr = self->rho + o; 

	*ptr = rho;
	
	ptr = self->u + 2*o; 

	*ptr++ = ux;
	*ptr   = uy;
}

void
LBD2Q9BGK_get_averages(const LBD2Q9BGK* self, int x, int y, int c,
				double* rho, double* ux, double* uy)
{
	const double* ptr;

	lb_assert(self != NULL);
	lb_assert(x >= 0 && x < NX);
	lb_assert(y >= 0 && y < NY);
	lb_assert(rho && ux && uy);

	const int o = OFFSET(x + 1, y + 1) + c;

	ptr = self->rho + o; 

	*rho = *ptr;

	ptr = self->u + 2*o; 

	*ux  = *ptr++;
	*uy  = *ptr;
}

void
LBD2Q9BGK_set_equilibrium(LBD2Q9BGK* self)
{
	int n;
	double* ptr;

	lb_assert(self != NULL);

	//lb_d2q9_BGK_setup_rho_guards(self);
	lb_d2q9_setup_rho_guards(&self->d2q9, self->rho);

	n = RANK*(NY + 2)*(NX + 2);
	ptr = self->pdfs[0];
	for (; n > 0; --n)
		*ptr++ = 0.0;

	for (n = 1; n < 9; ++n)
		memcpy(self->pdfs[n], self->pdfs[0],
		    RANK*(NX + 2)*(NY + 2)*sizeof(double));

	lb_d2q9_BGK_collide_pdfs(self, 1.0);	
}

void 
LBD2Q9BGK_collide_node(LBD2Q9BGK* self, int a, int x, int y, int c, double feq)
{
	lb_assert(self != NULL);
	lb_assert(x >= 0 && x < NX);
	lb_assert(y >= 0 && y < NY);

	const double ohm = 1./(self->parameters.tau);
	const double alp = 1. - 1./(self->parameters.tau);

	const int o = OFFSET(x + 1, y + 1) + c;
	const double tmp = self->pdfs[a][o];
	
	self->pdfs[a][o] = alp*tmp + ohm*feq;
}

void 
LBD2Q9BGK_stream(LBD2Q9BGK* self)
{
	lb_d2q9_stream_pdfs(&self->d2q9, self->pdfs);
}

void 
LBD2Q9BGK_average(LBD2Q9BGK* self)
{
	lb_d2q9_BGK_average_pdfs(self);
	lb_d2q9_setup_rho_guards(&self->d2q9, self->rho);
}

void 
LBD2Q9BGK_diff_opt(LBD2Q9BGK* self, int x, int y, int c, double *drx, double *dry, double *ddr)
{
	lb_d2q9_diff_opt(&self->d2q9, x, y, self->rho + c, drx, dry, ddr);
}

void
LBD2Q9BGK_advance(LBD2Q9BGK* self)
{
	lb_assert(self != NULL);

	lb_d2q9_stream_pdfs(&self->d2q9, self->pdfs);

	lb_d2q9_BGK_average_pdfs(self);
	//lb_d2q9_BGK_setup_rho_guards(self);
	lb_d2q9_setup_rho_guards(&self->d2q9, self->rho);
	lb_d2q9_BGK_collide_pdfs(self, self->parameters.tau);
}

void
LBD2Q9BGK_dump(const LBD2Q9BGK* self, const char* filename)
{
	int c;
	char d_name[20];
	
	hid_t file_id;
	hid_t file_space_id;
	hid_t mem_space_id;
	hid_t vel_space_id;

	hid_t rho_set_id[RANK];
	hid_t ux_set_id[RANK];
	hid_t uy_set_id[RANK];

#ifdef LB_ENABLE_MPI
	hid_t xfer_plist;
#else
# define xfer_plist H5P_DEFAULT
#endif /* LB_ENABLE_MPI */

	lb_assert(self != NULL);
	lb_assert(filename != NULL);

	/*
	 *  open the file
	 */

#ifdef LB_ENABLE_MPI
	{
		hid_t acc_tpl = H5Pcreate(H5P_FILE_ACCESS);

		H5Pset_fapl_mpio(acc_tpl, self->d2q9.comm, MPI_INFO_NULL);
		file_id = H5Fcreate(filename, H5F_ACC_TRUNC,
				    H5P_DEFAULT, acc_tpl);
		H5Pclose(acc_tpl);
	}
#else /* !LB_ENABLE_MPI */
	file_id = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
#endif /* ICHF_ENABLE_MPI */

	/*
	 *  create spaces
	 */

	{
#ifdef LB_ENABLE_MPI
		hsize_t gdims[2] = {self->d2q9.partition_info.global_size[0],
				    self->d2q9.partition_info.global_size[1]};
#else
		hsize_t gdims[2] = {NX, NY};
#endif /* LB_ENABLE_MPI */

		hsize_t ldims[2] = {NX + 2, RANK*(NY + 2)};
		hsize_t udims[2] = {NX + 2, 2*RANK*(NY + 2)};

		file_space_id = H5Screate_simple(2, gdims, NULL);
		mem_space_id = H5Screate_simple(2, ldims, NULL);
		vel_space_id = H5Screate_simple(2, udims, NULL);
	}

	/*
	 *  select memory slab
	 */

	{
		hsize_t start[2] = {1, RANK};
		hsize_t  count[2] = {NX, NY};
		hsize_t  stride[2] = {1, RANK};
		
		hsize_t start2[2] = {1, 2*RANK};
		hsize_t  count2[2] = {NX, NY};
		hsize_t  stride2[2] = {1, 2*RANK};

		H5Sselect_hyperslab(mem_space_id, H5S_SELECT_SET, 
					start, stride, count, NULL);
		H5Sselect_hyperslab(vel_space_id, H5S_SELECT_SET, 
					start2, stride2, count2, NULL);
	}

	/*
	 *  select slab in the file's data space
	 */

#ifdef LB_ENABLE_MPI
	{
		hsize_t start[2] = {self->d2q9.partition_info.global_origin[0],
				    self->d2q9.partition_info.global_origin[1]};
		hsize_t count[2] = {NX, NY};

		H5Sselect_hyperslab(file_space_id, H5S_SELECT_SET,
				    start, NULL, count, NULL);
	}
#endif /* LB_ENABLE_MPI */

for (c = 0; c < RANK; ++c){
	sprintf(d_name, "rho%d", c);
	rho_set_id[c] = H5Dcreate(file_id, d_name, H5T_NATIVE_DOUBLE, file_space_id, H5P_DEFAULT);
	sprintf(d_name, "ux%d", c);
	ux_set_id[c] = H5Dcreate(file_id, d_name, H5T_NATIVE_DOUBLE, file_space_id, H5P_DEFAULT);
	sprintf(d_name, "uy%d", c);
	uy_set_id[c] = H5Dcreate(file_id, d_name, H5T_NATIVE_DOUBLE, file_space_id, H5P_DEFAULT);
}

#ifdef LB_ENABLE_MPI
	xfer_plist = H5Pcreate(H5P_DATASET_XFER);
	H5Pset_dxpl_mpio(xfer_plist, H5FD_MPIO_COLLECTIVE);
#endif /* LB_ENABLE_MPI */

for (c = 0; c < RANK; ++c){
	H5Dwrite(rho_set_id[c], H5T_NATIVE_DOUBLE, mem_space_id,
			file_space_id, xfer_plist, self->rho + c);
	H5Dwrite(ux_set_id[c], H5T_NATIVE_DOUBLE, vel_space_id,
			file_space_id, xfer_plist, self->u + c + 0);
	H5Dwrite(uy_set_id[c], H5T_NATIVE_DOUBLE, vel_space_id,
			file_space_id, xfer_plist, self->u + c + 1);
}

#ifdef LB_ENABLE_MPI
	H5Pclose(xfer_plist);
#endif /* ICHF_ENABLE_MPI */

for (c = 0; c < RANK; ++c){
	H5Dclose(rho_set_id[c]);
	H5Dclose(ux_set_id[c]);
	H5Dclose(uy_set_id[c]);
}

	H5Sclose(file_space_id);
	H5Sclose(mem_space_id);
	H5Sclose(vel_space_id);
	H5Fclose(file_id);
}

void
lb_d2q9_stats(const LBD2Q9BGK* self, LBD2Q9BGKStats* stats)
{
	int x, y, o;
	LBD2Q9BGKStats my;

	lb_assert(self != NULL);

	my.min_density = 1.0e15;
	my.max_density = my.max_velocity =
	my.kin_energy = my.momentum_x = my.momentum_y = 0.0;

	for (x = 0, o = NY + 3; x < NX; ++x, o += 2)
		for (y = 0; y < NY; ++y) {
			const double r  = self->rho[o];
			const double ux = self->u[2*o+0];
			const double uy = self->u[2*o+1];

			const double u2 = ux*ux + uy*uy;
			const double u = sqrt(u2);

			my.min_density = LB_MIN(my.min_density, r);
			my.max_density = LB_MAX(my.max_density, r);
			my.max_velocity = LB_MAX(my.max_velocity, u);
			my.kin_energy += r*u2;
			my.momentum_x += r*ux;
			my.momentum_y += r*uy;
		}

#ifdef LB_ENABLE_MPI
        LB_CALL_MPI(MPI_Allreduce(&my.min_density, &stats->min_density,
		      1, MPI_DOUBLE, MPI_MIN, self->d2q9.comm))
        LB_CALL_MPI(MPI_Allreduce(&my.max_density, &stats->max_density,
		      1, MPI_DOUBLE, MPI_MAX, self->d2q9.comm))
        LB_CALL_MPI(MPI_Allreduce(&my.max_velocity,  &stats->max_velocity,
		      1, MPI_DOUBLE, MPI_MAX, self->d2q9.comm))
        LB_CALL_MPI(MPI_Allreduce(&my.kin_energy, &stats->kin_energy,
		      1, MPI_DOUBLE, MPI_SUM, self->d2q9.comm))
        LB_CALL_MPI(MPI_Allreduce(&my.momentum_x, &stats->momentum_x,
		      1, MPI_DOUBLE, MPI_SUM, self->d2q9.comm))
        LB_CALL_MPI(MPI_Allreduce(&my.momentum_y, &stats->momentum_y,
		      1, MPI_DOUBLE, MPI_SUM, self->d2q9.comm))

	{
		const int global_N =
			self->d2q9.partition_info.global_size[0]*
			self->d2q9.partition_info.global_size[1];

		stats->kin_energy /= 2*global_N;
		stats->momentum_x /= global_N;
		stats->momentum_y /= global_N;
	}
#else /* !LB_ENABLE_MPI */
	my.kin_energy /= 2.0*NX*NY;
	my.momentum_x /= NX*NY;
	my.momentum_y /= NX*NY;
        *stats = my;
#endif /* LB_ENABLE_MPI */
}

LBD2Q9BGKStats*
LBD2Q9BGK_stats(const LBD2Q9BGK* self)
{
	LBD2Q9BGKStats* stat;

	lb_assert(self != NULL);

	stat = lb_new(LBD2Q9BGKStats, 1);

	lb_d2q9_stats(self, stat);

	return stat;
}

double
LBD2Q9BGK_mass(const LBD2Q9BGK* self)
{
	int nx;
	double mymass = 0.0;
	const double* rho = self->rho + OFFSET(1, 1);

	lb_assert(self != NULL);

	for (nx = NX; nx > 0; --nx) {
		int ny = NY;
		for (; ny > 0; --ny) {
			mymass += *rho;
			rho++;
		}
		rho += 2;
	}

#ifdef LB_ENABLE_MPI
	{
		double totalmass;
		LB_CALL_MPI(MPI_Allreduce(&mymass, &totalmass, 1,
					MPI_DOUBLE, MPI_SUM, self->d2q9.comm))
		return totalmass;
	}
#else
	return mymass;
#endif /* LB_ENABLE_MPI */
}
