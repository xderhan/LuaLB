...
dump(simulation, pinfo, "start.dat")

for i = START, END do
	simulation:advance()
	M = simulation:mass()
	if rank == 0 and math.fmod(i, 16) == 0 then
		io.write(string.format("(%d) ==> mass = %f\n", i, M))
	end
end

dump(simulation, pinfo, "stop.dat")
