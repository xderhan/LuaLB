...
static inline void
lb_d3q19_Mix_average_pdfs_1(LBD3Q19Mix* self, int o)
{
...
lb_d3q19_Mix_average_pdfs(LBD3Q19Mix* self)
{
	...
	for (x = 1; x <= NX; ++x)
		for (y = 1; y <= NY; ++y)
			for (z = 1; z <= NZ; ++z)
				lb_d3q19_Mix_average_pdfs_1
					(self, ROFFSET(x, y, z));
}

static inline void
lb_d3q19_Mix_apply_bc(LBD3Q19Mix* self, int oo,
...
static inline void
lb_d3q19_Mix_collide_pdfs_1(LBD3Q19Mix* self, double rtau, double ptau, int oo,
					...
					const double* ddr, const double* ddp)
{
	...
	lb_d3q19_Mix_equilibrium(&self->parameters, self->rho + 2*oo,
...
static void
lb_d3q19_Mix_collide_pdfs(LBD3Q19Mix* self, double rtau, double ptau)
{
	...
	for (x = 1; x <= NX; ++x)
		for (y = 1; y <= NY; ++y)
			for (z = z0; z <= z1; ++z) {
				...
				lb_d3q19_Mix_collide_pdfs_1(self, rtau, ptau, oo, 
							&drx, &dry, &drz, 
							&dpx, &dpy, &dpz, &ddr, &ddp);
			}
	...
	if (z0 == 2)
		for (x = 1; x <= NX; ++x)
			for (y = 1; y <= NY; ++y) {
				...
				lb_d3q19_Mix_apply_bc(self, oo, self->uxuy_b5,
							10, 11, 14, 15);
	...
	if (z1 == NZ - 1)
		for (x = 1; x <= NX; ++x)
			for (y = 1; y <= NY; ++y) {
				...
				lb_d3q19_Mix_apply_bc(self, oo, self->uxuy_b4,
							12, 13, 14, 17);
...
#ifdef LB_ENABLE_MPI
static void
lb_d3q19_Mix_exchange_rho(LBD3Q19Mix* self)
{
...
static inline void
lb_d3q19_Mix_copy_X_edge(LBD3Q19Mix* self, double* dst, const double* src)
{
...
static inline void
lb_d3q19_Mix_copy_Y_edge(LBD3Q19Mix* self, double* dst, const double* src)
{
...
static inline void
lb_d3q19_Mix_copy_Z_edge(LBD3Q19Mix* self, double* dst, const double* src)
{
	memcpy(dst, src, NZ*sizeof(double));
}

static inline void
lb_d3q19_Mix_copy_XY_plane(LBD3Q19Mix* self, double* dst, const double* src)
{
	const int stride = (NY + 2)*(NZ + 2);
	int count = NX;

	for (; count > 0; --count) {
		lb_d3q19_Mix_copy_Y_edge(self, dst, src);
		dst += stride;
		src += stride;
	}
}
...
static inline void
lb_d3q19_Mix_copy_XZ_plane(LBD3Q19Mix* self, double* dst, const double* src)
{
	const int stride = (NY + 2)*(NZ + 2);
	int count = NX;

	for (; count > 0; --count) {
		lb_d3q19_Mix_copy_Z_edge(self, dst, src);
		dst += stride;
		src += stride;
	}
}
...
static inline void
lb_d3q19_Mix_copy_YZ_plane(LBD3Q19Mix* self, double* dst, const double* src)
{
	const int stride = NZ + 2;
	int count = NY;
...
static void
lb_d3q19_Mix_wrap_rho_X(LBD3Q19Mix* self)
{
	lb_d3q19_Mix_copy_YZ_plane(self, self->rho + OFFSET(0, 1, 1),
					   self->rho + OFFSET(NX, 1, 1));
	lb_d3q19_Mix_copy_YZ_plane(self, self->rho + OFFSET(NX + 1, 1, 1),
					   self->rho + OFFSET(1, 1, 1));
}

static void
lb_d3q19_Mix_wrap_rho_Y(LBD3Q19Mix* self)
{
	lb_d3q19_Mix_copy_XZ_plane(self, self->rho + OFFSET(1, 0, 1),
					   self->rho + OFFSET(1, NY, 1));
	lb_d3q19_Mix_copy_XZ_plane(self, self->rho + OFFSET(1, NY + 1, 1),
					   self->rho + OFFSET(1, 1, 1));
}

static void
lb_d3q19_Mix_wrap_rho_Z(LBD3Q19Mix* self)
{
	lb_d3q19_Mix_copy_XY_plane(self, self->rho + OFFSET(1, 1, 0),
					   self->rho + OFFSET(1, 1, NZ));
	lb_d3q19_Mix_copy_XY_plane(self, self->rho + OFFSET(1, 1, NZ + 1),
					   self->rho + OFFSET(1, 1, 1));
}

static void
lb_d3q19_Mix_wrap_rho(LBD3Q19Mix* self)
{
#ifdef LB_ENABLE_MPI
...
{
#ifdef LB_ENABLE_MPI
	lb_d3q19_Mix_exchange_rho(self);
#endif /* LB_ENABLE_MPI */
	lb_d3q19_Mix_wrap_rho(self);
}
...
LBD3Q19Mix*
LBD3Q19Mix_new(int nx, int ny, int nz, int pz)
{
	int nnodes, n;
	LBD3Q19Mix* self;
	...
	self->parameters.Gr = 1.0;
	self->parameters.lamda = 1.1;
	self->parameters.rtau = 0.8;
	self->parameters.ptau = 0.8;

	if (NX < 3 || NY < 3 || NZ < 3)
		lb_fatal("D3Q19Mix: size of the grid too small"
#ifdef LB_ENABLE_MPI
...
void
LBD3Q19Mix_destroy(LBD3Q19Mix* self)
{
	int n;
	lb_assert(self != NULL);
...
LBPartitionInfo*
LBD3Q19Mix_partition_info(const LBD3Q19Mix* self)
{
	LBPartitionInfo* pinfo;
	...
	_LBD3Q19Mix_partition_info(self, pinfo);

	return pinfo;
}

void
LBD3Q19Mix_set_parameters(LBD3Q19Mix*            self,
			      const LBMixParameters* parameters)
{
	lb_assert(self != NULL);
	lb_assert(parameters != NULL);
	lb_assert(parameters->T > 0.0);
	lb_assert(parameters->a >= 0.0);
	lb_assert(parameters->b >= 0.0);
	lb_assert(parameters->K > 0.0);
	lb_assert(parameters->Gr > 0.0);
	lb_assert(parameters->lamda > 0.0);
...
LBMixParameters*
LBD3Q19Mix_get_parameters(const LBD3Q19Mix* self)
{
	LBMixParameters* params;

	lb_assert(self != NULL);

	params = lb_new(LBMixParameters, 1);

	_LBD3Q19Mix_get_parameters(self, params);
			       ...
			       const double v5[2])
{
	...
	if (self->d3q19.partition_info.periods[2])
		lb_warning("LBD3Q19Mix: setting speed of the walls will"
...
void
LBD3Q19Mix_set_averages(LBD3Q19Mix* self, int x, int y, int z,
...
void
LBD3Q19Mix_get_averages(const LBD3Q19Mix* self, int x, int y, int z,
			    double* rho, double* phi, double* ux, double* uy, double* uz)
{
	int o;
...
void
LBD3Q19Mix_set_equilibrium(LBD3Q19Mix* self)
{
	int n, N;
	double* ptr;
	...
	//lb_d3q19_Mix_setup_rho_guards(self);
	...
	lb_d3q19_Mix_collide_pdfs(self, 1.0, 1.0);	
}

void
LBD3Q19Mix_advance(LBD3Q19Mix* self)
{
	lb_assert(self != NULL);

	lb_d3q19_stream_pdfs(&self->d3q19, self->pdfs);

	//lb_d3q19_Mix_average_pdfs(self);
	...
	lb_d3q19_Mix_collide_pdfs(self, 
				self->parameters.rtau,
				self->parameters.ptau);
}

void
LBD3Q19Mix_dump(const LBD3Q19Mix* self, const char* filename)
{
	hid_t file_id;
	hid_t file_space_id;
	hid_t mem_space_id;
...
LBD3Q19MixStats*
LBD3Q19Mix_stats(const LBD3Q19Mix* self)
{
	LBD3Q19MixStats* stat;

	lb_assert(self != NULL);

	stat = lb_new(LBD3Q19MixStats, 1);

	_LBD3Q19Mix_stats(self, stat);

	return stat;
}

double
LBD3Q19Mix_mass(const LBD3Q19Mix* self)
{
	int x, y, z;
	double mymass = 0.0;
