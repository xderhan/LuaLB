...
static void
lb_d2q9_Mix_average_pdfs_1(LBD2Q9Mix* self, int o)
{
...
lb_d2q9_Mix_average_pdfs(LBD2Q9Mix* self)
{
	...
	for (x = 1; x <= NX; ++x)
		for (y = 1; y <= NY; ++y)
			lb_d2q9_Mix_average_pdfs_1(self, OFFSET(x, y) );
}

#ifdef LB_ENABLE_MPI
static void
lb_d2q9_Mix_exchange_rho(LBD2Q9Mix* self)
{
...
static void
lb_d2q9_Mix_wrap_rho(LBD2Q9Mix* self)
{
	lb_assert(self != NULL);
...
#ifdef LB_ENABLE_MPI
	lb_d2q9_Mix_exchange_rho(self);
#endif /* LB_ENABLE_MPI */
	lb_d2q9_Mix_wrap_rho(self);
}

static inline void
lb_d2q9_Mix_collide_node_pdfs(LBD2Q9Mix* self, int o,
				...
				const double* LB_RESTRICT ptau)
{
	...
	lb_d2q9_Mix_equilibrium(&self->parameters,
				  self->rho + o, self->u + o,
...
static inline void
lb_d2q9_Mix_apply_bc(double* LB_RESTRICT averages,
...
static void
lb_d2q9_Mix_collide_pdfs(LBD2Q9Mix* self, double rtau, double ptau)
{
	...
	for (x = 1; x <= NX; ++x)
		for (y = y0; y <= y1; ++y) {
			...
			lb_d2q9_Mix_collide_node_pdfs(self, offset,
					    &drx, &dry, &dpx, &dpy, 
						&ddr, &ddp, &rtau, &ptau);
		}
	...
	if (lb_d2q9_is_boundary(&self->d2q9, 3))
		for (x = 1; x <= NX; ++x) {
			...
			lb_d2q9_Mix_apply_bc(self->rho + offset,
						   self->u + offset,
					       self->pdfs, offset,
					       &self->ux_b3, 4, 5);
...
LBD2Q9Mix*
LBD2Q9Mix_new(int nx, int ny, int py)
{
	int n, nnodes;
	LBD2Q9Mix* self;
	...
	if (NX < 3 || NY < 3)
		lb_fatal("D2Q9Mix: size of the grid too small"
#ifdef LB_ENABLE_MPI
...
void
LBD2Q9Mix_destroy(LBD2Q9Mix* self)
{
	int n;
...
LBPartitionInfo*
LBD2Q9Mix_partition_info(const LBD2Q9Mix* self)
{
	LBPartitionInfo* pinfo;
	...
	_LBD2Q9Mix_partition_info(self, pinfo);

	return pinfo;
}

void
LBD2Q9Mix_set_parameters(LBD2Q9Mix*             self,
...
LBMixParameters*
LBD2Q9Mix_get_parameters(const LBD2Q9Mix* self)
{
	...
	_LBD2Q9Mix_get_parameters(self, params);

	return params;
}

void
LBD2Q9Mix_set_walls_speed(LBD2Q9Mix* self, double vx_top, double vx_down)
{
	lb_assert(self != NULL);

	if (self->d2q9.partition_info.periods[1])
		lb_warning("LBD2Q9Mix: setting speed of the walls will"
...
void
LBD2Q9Mix_get_walls_speed(const LBD2Q9Mix* self,
...
void
LBD2Q9Mix_set_averages(LBD2Q9Mix* self, int x, int y,
...
void
LBD2Q9Mix_get_averages(const LBD2Q9Mix* self, int x, int y,
			   double* rho, double* phi, double* ux, double* uy)
{
	const double* ptr;
...
void
LBD2Q9Mix_set_equilibrium(LBD2Q9Mix* self)
{
	int n;
	double* ptr;
	...
	//lb_d2q9_Mix_setup_rho_guards(self);
	...
	lb_d2q9_Mix_collide_pdfs(self, 1.0, 1.0);	
}

void
LBD2Q9Mix_advance(LBD2Q9Mix* self)
{
	lb_assert(self != NULL);

	lb_d2q9_stream_pdfs(&self->d2q9, self->pdfs);

	//lb_d2q9_Mix_average_pdfs(self);
	...
	lb_d2q9_Mix_collide_pdfs(self, 
				self->parameters.rtau, 
				self->parameters.ptau);
}

void
LBD2Q9Mix_dump(const LBD2Q9Mix* self, const char* filename)
{
	hid_t file_id;
	hid_t file_space_id;
	hid_t mem_space_id;
...
LBD2Q9MixStats*
LBD2Q9Mix_stats(const LBD2Q9Mix* self)
{
	LBD2Q9MixStats* stat;

	lb_assert(self != NULL);

	stat = lb_new(LBD2Q9MixStats, 1);

	_LBD2Q9Mix_stats(self, stat);

	return stat;
}

double
LBD2Q9Mix_mass(const LBD2Q9Mix* self)
{
	int nx;
	double mymass = 0.0;
